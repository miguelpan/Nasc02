...
public class SellerDaoJDBC implements SellerDao {
	...
	public void insert(Seller obj) {
		...
		try {
			st = conn.prepareStatement(
					"INSERT INTO seller "
					+ "(Name, Email, BirthDate, BaseSalary, FuncaoId) "
					+ "VALUES "
					+ "(?, ?, ?, ?, ?)",
			...
			st.setInt(5, obj.getFuncao().getId());
	...
	public void update(Seller obj) {
		...
		try {
			st = conn.prepareStatement(
					"UPDATE seller "
					+ "SET Name = ?, Email = ?, BirthDate = ?, BaseSalary = ?, FuncaoId = ? "
					+ "WHERE Id = ?");
	...
	public Seller findById(Integer id) {
		...
		try {
			st = conn.prepareStatement(
					...
					+ "ON seller.FuncaoId = department.Id "
					+ "WHERE seller.Id = ?");
	...
	private Seller instantiateSeller(ResultSet rs, Funcao dep) throws SQLException {
	...
	@Override
	public List<Seller> findByFuncao(Funcao department) {
		PreparedStatement st = null;
		ResultSet rs = null;
		try {
			st = conn.prepareStatement(
